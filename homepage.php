<!doctype html>
<html lang="en">
<head>
    <title>Title</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
          integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
</head>
<body>
<div>
    <img class="d-block w-100" src="img/sach5.png" alt="First slide">
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <a class="navbar-brand" href="index1.php">
            <nav class="navbar navbar-light bg-light">
                <a class="navbar-brand" href="index1.php">
                    <img src="img/logo2.png" width="100px" height="100px" alt="">
                </a>
            </nav>
        </a>
        <button class="navbar-toggler" type="button" data-toggle="collapse"
                data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                <li class="nav-item active">
                    <a class="nav-link" href="index1.php">Home <span class="sr-only">(current)</span></a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="">Link</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="login.php">Login</a>
                </li>
                    <form class="form-inline">
                        <input class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search">
                        <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
                    </form>
            </ul>

        </div>
    </nav>

<div>
    <div class="p-lg-5">

        <div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img class="d-block w-100" src="img/case1.jpg" alt="First slide">
                </div>
                <div class="carousel-item">
                    <img class="d-block w-100" src="img/case2.png" alt="Second slide">
                </div>
                <div class="carousel-item">
                    <img class="d-block w-100" src="img/case3.jpg" alt="Third slide">
                </div>
            </div>
            <a class="carousel-control-prev" href="#carouselExampleControls" role="button"
               data-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="sr-only">Previous</span>
            </a>
            <a class="carousel-control-next" href="#carouselExampleControls" role="button"
               data-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                <span class="sr-only">Next</span>
            </a>
        </div>

    </div>

</div>

    <div>
        <nav class="pl-5" style=";float: left; max-width: 160px;margin: 0;padding: 1em">
            <p>Danh mục</p>
            <?php foreach ($categories as $category): ?>
                <div>
                    <ul style="list-style-type: none;padding: 0">
                        <li>
                            <a style="text-decoration: none" href="index1.php?page=view-book&category=<?php echo $category->getId()?>"><?php echo $category->getCategoryName() ?></a>
                        </li>
                    </ul>
                </div>
            <?php endforeach; ?>
        </nav>
    </div>
    <div >
        <article style="margin-left: 170px;border-left: 1px solid gray;padding: 1em;overflow: hidden">
            <header role="banner" style=" padding: 1em;color: white;background-color: black;clear: left;text-align: center">
                <h1>Sách</h1>
            </header>
            <div style="border-left: 1px solid gray;">
                <div class="container-fluid" style="width: 1500px">
                    <?php foreach ($books as $book): ?>
                        <div class="card mt-2 ml-2"
                             style="display: inline-block;width: 270px;height: 600px;float: left">
                            <img class="card-img-top" src="<?php echo $book->getImage() ?>" alt="Card image cap">
                            <div class="card-body" style="height: 135px">
                                <div style="height: 80px">
                                    <h5 class="card-title"><?php echo $book->getBookName() ?></h5>
                                </div>
                                <div>
                                    <p class="card-text">$: <?php echo $book->getUnitPrice() ?> VNĐ</p>
                                </div>
                            </div>

                            <div class="card-footer">
                                <a href="#" class="btn btn-primary" style="margin-bottom: 10px">Buy</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
        </article>
    </div>
    <img class="d-block w-100" src="img/sach5.png" alt="First slide">

    <!---->
    <!---->
    Optional JavaScript
    jQuery first, then Popper.js, then Bootstrap JS
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"
            integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo"
            crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"
            integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1"
            crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"
            integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM"
            crossorigin="anonymous"></script>
</body>
</html>